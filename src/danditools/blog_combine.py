...
def get_reciever_div(soup):
    first = soup.findChildren(
        partial(filter_for_attr_val, attr="data-hook", value="rcv-block-first")
    )
    last = soup.findChildren(
        partial(filter_for_attr_val, attr="data-hook", value="rcv-block-last")
    )
    ...
    for child in first.next_siblings:
        ...
        if reciever_div is not None:
            raise IncorrectAssumptionError(
                "I thought that there would be one of these"
            )
        reciever_div = child
...
def main():
    ...
    while parsed.to_combine:
        if soup is None:
            soup = BeautifulSoup(
                parsed.to_combine.pop(0).read_text(), features="html.parser"
            )
    ...
    def checker(element):
        ...
        if not res and (
            "recent" in str(element).lower() or "views" in str(element).lower()
        ):
            import pdb
    ...
    for rp in soup.find_all(
        lambda ch: ch.attrs.get("data-hook") in ("post-footer", "recent-posts")
    ):
