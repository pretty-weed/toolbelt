...
def create_pages(source = None, dest="-spreads", front_cover=True, back_cover=True, pad_spreads=False, dest_is_suffix=True):
  ...
  scribus.newDocument((source_size[0]*2, source_size[1]), source_margins, scribus.PORTRAIT, 1, unit, scribus.PAGE_1, 0, offset)
  scribus.saveDocAs(dest)
  scribus.setDocType(scribus.NOFACINGPAGES, scribus.FIRSTPAGELEFT)
  print("boop")
  print(spread_sources[::2], spread_sources[1::2], range(len(spread_sources) // 2))
  print(f"rangeval: {len(spread_sources) // 2}")
  print(list((list(range(1+offset, 2+(len(spread_sources) // 2))), spread_sources[::2], spread_sources[1::2])))
  print(list(zip(range(1+offset, 2+(len(spread_sources) // 2)), spread_sources[::2], spread_sources[1::2])))
  for new_page, left_source, right_source in zip(range(1+offset, 2+((len(spread_sources)) // 2)), spread_sources[offset::2], spread_sources[offset+1::2]):
    ...
    copy_items(CopySrc(source, right_source), CopyDest(dest, new_page), (source_size[0], 0))
  # Do covers
  if front_cover:
    ...
    print(f"setting front cover size. {scribus.currentPage()}:{scribus.getDocName()}")
    scribus.setCurrentPageSize(*source_size)
    print(f"after set page size: {scribus.getPageSize()}")
  if back_cover:
    scribus.newPage(-1)
    copy_items(CopySrc(source, spread_sources[-1]), CopyDest(dest, scribus.pageCount()))
    if scribus.getDocName() != dest:
      print(f"Have to go to new doc")
      scribus.openDoc(dest)
...
create_pages(r"C:\Users\me\zines\time_travellers_guide_resisting_extremism\zine.sla")
