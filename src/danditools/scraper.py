...
def main():
    ...
    parser.add_argument("--target_regex", "--TR", action="append", type=re.compile)
    parser.add_argument("--target_ext", "-T", action="append", dest="target_exts")
    ...
    print(parsed)
    for link_title, link in get_download_links(parsed.start, parsed.target_exts, parsed.target_regex, follow=parsed.follow):
        download_file(link, parsed.out_dir, link_title)

def get_download_links(
        start_pages: list[str], target_exts: Collection[str]=None, target_res: Collection[re.Pattern]=None,
        checked: Optional[set[str]] = None, 
        follow: bool | int = False) -> Iterator[str]:

    if not any([target_exts, target_res]):
        raise TypeError("Either `target_exts` or `target_res` must be provided.")
        ...
        for tag in soup.find_all("a", href=True):
            ...
            if follow and href.endswith((".htm", ".html", ".php", "/")):
                if follow is not True:
                    follow -= 1
                try:
                    yield from get_download_links(href, checked=checked, follow=follow)
                except HTTPError:
                    continue
            if not href.startswith("http"):
                import pdb
                pdb.set_trace()
            if href.endswith(target_exts) or any(tre.match(href) for tre in (target_res or [])):
...
def download_file(url, download_dir: pathlib.PurePath, base_filename: str = None):
    ...
    if base_filename is None:
        base_filename = url.split("/")[-1].rsplit("?", 1)[0]
    base_filename = pathlib.Path(base_filename)
    filepath = download_dir.joinpath(base_filename)
    counter = 1

    while filepath.exists():
        filepath = filepath.with_name(f"{base_filename}_{counter}").with_suffix(base_filename.suffix)
        counter += 1
