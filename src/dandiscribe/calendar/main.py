...
basicConfig(filename=str(LOG_PATH), encoding='utf-8', level=INFO, format='%(asctime)s %(levelname)-8s %(message)s',)
...
from dandiscribe.util import cache_val, clear_cache_val, get_cache_val, ok_to_ignore_dialog, Debug, MISSING, PauseDrawing, TempGoTo

import dandiscribe.calendar.data as data
from dandiscribe.calendar.pages import gen_notes_spread_pages, A5FrontPage, A5MonthSpreadPage, A5NotesPage, A5WeekSpreadPage, MonthSpreadPage, WeekSpreadPage
...
class Document:
    ...
    def draw(self, tasks: list[Task] | None = None, events: list[Event] | None = None):
        ...
        for page in self.pages:
            scribus.progressSet((done + 1) // total)
            page.draw(master=False, tasks=tasks, events=events.get(page.page_date))
            done += 1
...
def prompt_to_date(prompt_str, start_date: datetime.datetime):
...
def make_doc(routines_file=ROUTINES_FILE) -> Document:
    ...
    first_page_date = datetime.date.today()
    end = datetime.date(first_page_date.year, 12, 31)

    cached_prompt = get_cache_val("dates_prompt")
    dates_prompt = cached_prompt if cached_prompt is not MISSING else end.isoformat()

    logger.debug("Init dates prompt: %s", dates_prompt)
    dates_prompt = scribus.valueDialog(
        "date span/last date", f'{first_page_date.strftime("%Y/%m/%d")}-{end.strftime("%Y/%m/%d")}',dates_prompt 
    )
    ...
    while dates_prompt:
        ...
        if dates_prompt.count("-") % 2:
            # multiple dates:
            start_prompt, end_prompt  = '/'.join(dates_prompt.split('-')[:len(dates_prompt.split('-'))// 2]), '/'.join(dates_prompt.split('-')[len(dates_prompt.split('-'))//2:])
        try:    
    ...
    for event in Event.get_from_calendars(
            datetime.datetime.combine(first_page_date, datetime.time.min),
        ...
        try:
            ...
            start_time, end_time = datetime.time.min, datetime.time.max
        else:
        ...
        for day_n in range((end_date - start_date).days + extra_page):
            event_by_date.setdefault(start_date + datetime.timedelta(days=day_n), []).append(event)
        else:
            continue
    ...
    while page_date <= end:
        ...
        if page_date.month != pages[-1].page_date.month and page_date.month > first_page_date.month:
            pages += gen_notes_spread_pages(len(pages) + 1, factory=A5NotesPage) + [
                A5MonthSpreadPage(
